...
$.fn.spy = function(settings) {
	...
	spy.timestamp = function() {
	    var now = new Date();
		return (Math.floor(now / 1000))-900;
	}
	...
	spy.parse = function(e, r) {
		...
		//Added to work with SimpleGeo API
		spy.json = spy.json.features;
		if (spy.json && (o.method == 'json' && spy.json.constructor == Array) || o.method == 'html') {
		    //Sort by time
		    spy.json.sort(function(a,b)
		    {
	...
	return this.each(function() {
		...
		spy.ajaxTimer = window.setTimeout(function() {
		    var timestamp = o.timestamp.call();
			if (spyRunning && (!spy.parsing)) {
				$.get(o.ajax, { 'start': timestamp }, function(r) {
					spy.parse(e, r);
				});
			}
